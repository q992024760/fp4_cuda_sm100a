...
#else
#define BLOCKSCALE_NX 4   //scale_vec::4X
#endif
...
union InstrDescriptorBlockScaled
{
  ...
  struct {
    ...
    uint16_t sparse_id2_    : 2,  // bit [ 0, 2) : Sparse meta data id2
             ...
             sat_int_type   : 1,  // bit [ 3, 4)
             d_type         : 2,  // bit [ 4, 6) : Matrix B Scale Factor ID
                            : 1,  //
    ...
    uint16_t b_major_       : 1,  // bit [16,17) : 0 = K-major. 1 = MN-major. Major value of 1 is only valid for E4M3, E5M2, INT8 (signed and unsigned), F16, BF16 and TF32 source formats
             ...
             m_dim_         : 5,  // bit [24,29) : 4 LSBs not included. Valid values are: 4 (M=64), 8 (M=128), 16 (M=256)
                            : 1,  // bit [29,30) 
             b_matrix       : 2;  // bit [30,32) : Matrix A Scale Factor ID
  };
...
__global__ void mma_on_tmem(uint8_t *mat_a, uint8_t *mat_b, float *mat_c) {
  int tid     = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t  mat_a_share[M * 64];
  ...
  for(int k_loop = 0; k_loop < K; k_loop += 64) {
    if(tid==0){
      for (int i = 0; i < M; i++){
        for (int j = 0; j < 64; j++){
          if ((i/4)%2==0){
            mat_a_share[i*64 +j] = mat_a[i*K +j + k_loop];
          }else{
            int j_ = (j+16)%64;
            mat_a_share[i*64 +j] = mat_a[i*K +j_ + k_loop];
          }
        }
      }
      ...
      for (int i = 0; i < M; ++i) {
        printf(":[ ");
        for (int k = 0; k < K; k++) {
          printf("%x  ",(((uint8_t *)mat_a_share)[i * K + k]));
        }
        printf(" ]\n");
      }
    }
    ...
    mat_a_desc.leading_byte_offset_ = 32>>4;    //当前线程两个128bit之间地址差
...
int main() {
    ...
    void *host_A = nullptr, *host_B = nullptr, *host_C = nullptr;
    cudnnMallocHostInBytes(&host_A, M * K * sizeof(uint8_t));
    ...
    for (int i = 0; i < M; ++i) {
      for (int k = 0; k < K; k++) {
        ...
        // ((uint8_t *)host_A)[i * K + k] = rand() % 256;
        ((uint8_t *)host_A)[i * K + k] = 1 << 3;
      }
    }
    ...
    //print
    {
      ...
      if(SCALE_FORMAT) {
        std::cout <<"Scale:e8m0"<<std::endl;
      } else {
        std::cout <<"Scale:e4m3"<<std::endl;
      }
      ...
      for (int i = 0; i < M; ++i) {
        ...
        for (int k = 0; k < K; k++) {
          ...
          std::cout << std::fixed << std::setprecision(1) << (((uint8_t *)host_A)[i * K + k]);
          std::cout <<" ";
        }
        std::cout <<"]"<<std::endl;
      }
      ...
      for (int i = 0; i < M; ++i) {
        std::cout << std::setw(3) <<i<<":[ ";
        for (int k = 0; k < K; k++) {
          printf("0x%02X , ", ((uint8_t *)host_A)[i * K + k]);
        }
        std::cout <<"]"<<std::endl;
      }
    ...
    cudaMalloc(&dev_A, M * K * sizeof(uint8_t));
    cudaMalloc(&dev_B, N * K * sizeof(uint8_t) / 2);
    cudaMalloc(&dev_C, M * N * sizeof(float));
    checkCUDAErrors(cudaMemcpy(dev_A, host_A, M * K * sizeof(uint8_t), cudaMemcpyHostToDevice));
