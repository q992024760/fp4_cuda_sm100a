...
__global__ void mma_on_tmem(uint8_t *mat_a, uint8_t *mat_b, uint8_t *mat_sfa, uint8_t *mat_sfb, float *mat_c) {
    ...
    __shared__ uint8_t  mat_a_share[M * K / 2];
    ...
    for (int i = 0; i < M; i++){
      for (int j = 0; j < K / 2; j++){
        if ((i/4)%2==0){
          mat_a_share[i*K/2 +j] = mat_a[i*K/2 +j];
        }else{
          int j_ = (j+16)%32;
          mat_a_share[i*K/2 +j] = mat_a[i*K/2 +j_];
        }
      }
    }
    ...
    for (int i = 0; i < N; i++){
      for (int j = 0; j < K / 2; j++){
        if ((i/4)%2==0){
          mat_b_share[i*K/2 +j] = mat_b[i*K/2 +j];
        }else{
          int j_ = (j+16)%32;
          mat_b_share[i*K/2 +j] = mat_b[i*K/2 +j_];
        }
      }
    }


    for (int i = 0; i < M * 16; ++i) sfa_share[i] = (uint8_t)0;
    for (int i = 0; i < M * 16; ++i) sfb_share[i] = (uint8_t)0;
...
int main() {
    ...
    for (int i = 0; i < N; ++i) {
      for (int j = 0; j < BLOCKSCALE_NX; j++) {
        ((uint8_t *)host_sfb)[i*16 + j] = (uint8_t)(e4m3_valid_vals[rand()%239]);//without nan/inf
        // ((uint8_t *)host_sfa)[i*16 + j] = (uint8_t)(rand()%254);
    ...
    //print
    {
      ...
      for (int i = 0; i < N; ++i) {
        ...
        std::cout <<"scale A (scale_vec::"<<BLOCKSCALE_NX<<"X 、e4m3) 0x:"<<std::endl;
      for (int i = 0; i < M; ++i) {
        std::cout << std::setw(3) <<i<<":[ ";
        for (int j = 0; j < BLOCKSCALE_NX; j ++) {
        ...
        std::cout <<"scale B (scale_vec::"<<BLOCKSCALE_NX<<"X 、e4m3) 0x:"<<std::endl;
      for (int i = 0; i < N; ++i) {
        std::cout << std::setw(3) <<i<<":[ ";
        for (int j = 0; j < BLOCKSCALE_NX; j ++) {
    ...
    std::cout <<"Mat_C 0x:"<<std::endl;
    for(int m = 0; m < M; m++) {
      printf("%3d : ",m);
      for(int n = 0; n < N; n++) {
        uint32_t bits;
        memcpy(&bits, &((float *)host_C)[m * N + n], sizeof(float));
        printf("0x%08X ,", bits);
        // printf("0x%08X , ", (uint32_t)((float *)host_C)[m * N + n]);
      }
      printf("\n");
    }
