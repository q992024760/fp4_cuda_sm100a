...
__global__ void mma_on_tmem(uint8_t *mat_a, uint8_t *mat_b, uint8_t *mat_sfa, uint8_t *mat_sfb, float *mat_c) {
  ...
  for(int k_loop = 0; k_loop < K; k_loop += 64) {
    ...
    desc.a_format_ = 1;
    desc.b_format_ = 1;
    desc.scale_format_ = SCALE_FORMAT;  // 0=E4M3, 1=E8M0
    desc.a_major_ = 0;
    desc.b_major_ = 0;
    desc.a_negate_ = 0;
    desc.b_negate_ = 0;
...
int main() {
    ...
    for(int k = 0; k < (K/16); k+=4){ 
      for (int i = 0; i < M; ++i) {
        for (int j = 0; j < BLOCKSCALE_NX; j++) {
          ((uint8_t *)host_sfa)[i*MAX(16,(K/16)) + k + j] = (e4m3_valid_vals[rand()%120]);
      ...
      for (int i = 0; i < N; ++i) {
        for (int j = 0; j < BLOCKSCALE_NX; j++) {
          ((uint8_t *)host_sfb)[i*MAX(16,(K/16)) + k + j] = (e4m3_valid_vals[rand()%120]);
