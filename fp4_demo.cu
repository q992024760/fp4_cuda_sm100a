...
#define N 32
#define K 64
#define BLOCKSCALE_NX 4   //scale_vec::2X //指令内部分手动设置
...
int main() {
    ...
    for (int i = 0; i < M; ++i) {
      for (int j = 0; j < BLOCKSCALE_NX; j++) {
        // ((uint8_t *)host_sfa)[i*16 + j] = (uint8_t)(127+i);
    ...
    for (int i = 0; i < N; ++i) {
      for (int j = 0; j < BLOCKSCALE_NX; j++) {
        ((uint8_t *)host_sfb)[i*16 + j] = (uint8_t)(127+i);
    ...
    //   for (int k = 0; k < K; k += 2) {
    //     uint8_t val = i & 0x0F;
    //     // ((uint8_t *)host_A)[i * (K/2) + k/2] = (val << 4) | val;
    //     ((uint8_t *)host_A)[i * (K/2) + k/2] = 34;
    //   }
    // }
    ...
    //   for (int k = 0; k < K; k += 2) {
    //     uint8_t val = i & 0x0F;
    //     // ((uint8_t *)host_B)[i * (K/2) + k/2] = (val << 4) | val;
    //     ((uint8_t *)host_B)[i * (K/2) + k/2] = 34;
    //   }
    // }
